...
- 2026-05-04: API path configuration is now centralized in `src/api/core/endpoints.ts` (version-controlled endpoint registry); auth/admin adapters no longer read path env vars, reducing runtime config drift and making endpoint evolution reviewable in code.
...
- 2026-05-05: API contract sync now uses centralized enum-like constants in `src/api/core/enums.ts` (account/job/system-user/device platform) and wrappers/components consume these typed values instead of raw numeric literals.
...
- 2026-05-05: All `src/api` wrapper endpoint paths were centralized in `src/api/core/endpoints.ts` so OpenAPI diff tooling can deterministically map and validate every used endpoint call.
...
- 2026-05-07: Worker portal IA was restructured to PRD's 6 main groups (`Panel / İş Bul / Vardiyalarım / Cüzdan / Profil / Bildirimler`). New parent pages (`JobsPage`, `MyShiftsPage`, `WalletPage`, `NotificationsPage`) reuse existing screens (`Recommendations`, `Applications`, `Shifts`, `QrCheck`, `Payouts`, `Reports`, `CvImport`) as `embedded` tabs to avoid duplication. Old URLs (`/worker/applications`, `/recommendations`, `/qr-check`, `/payouts`, `/reports`, `/cv-import`) redirect to the new IA.
- 2026-05-07: `worker-ui.tsx` now exports `WorkerTabs`, `WorkerNotice`, and `WorkerNavBadge` Ocean primitives (8pt grid, focus-ring `cyan-400/45`, RTL-friendly logical paddings) so all new tabbed parent pages and the email verification banner share one design language.
- 2026-05-07: `WorkerLayout` shows a sticky email-verification banner for `AccountStatus.Pending` accounts and hosts dynamic sidebar badges (`pendingPayouts` on Wallet, `newMatches` on Find Jobs) via the new `useWorkerLiveCounters` hook (60s revalidate, graceful 0 fallback). Account status and `systemUserId` are resolved from `SystemUsers/Me`.
- 2026-05-07: `workerPortalApi` was extended without touching existing methods: `getReliabilityScore` (derived from accepted/rejected/expired ratio), `getUpcomingShiftAssignments`, `getActiveShiftAssignment`, `listShiftHistory` (all from `ShiftAssignments/MyAssignments`), `listNotifications` (mapped from `Workers/LiveStatus`), `getLiveCounters`, `getAvailabilityCalendar` (from `Workers/GetSelfFullDetail.availabilities`, ready for TimeSlot endpoint swap), and `requestEmailVerification` (proxy over `SystemUsers/RequestEmailVerification` with client-generated token + 1h expiry).
- 2026-05-07: `/worker/shifts` URL semantically changed to "My Shifts"; the previous open-jobs listing is now reachable via `/worker/jobs?tab=open` (or the default `recommendations` tab). Old bookmarks resolve via redirect rules in `WorkerDashboard`.
- 2026-05-07: Added shared `HeaderUserMenu` (`src/shared/ui/HeaderUserMenu.tsx`) wired into Worker/Employer topbars and admin `Navbar` (when `showSidebarToggle && isAuthenticated`). The header user icon now opens a single context popover containing profile shortcut, theme switch (Dark/Light), language switch (all `SUPPORTED_LOCALES`), and logout. New i18n namespace `dashboard.userMenu` added across `tr/en/es/it/ru/ar/fr`. Outside-click + ESC close the popover; alignment respects RTL via logical `start/end`.
- 2026-05-07: Project-wide structural refactor to feature-first + folder-per-page. New top-level folders: `src/shared/{ui,lib}` (reusable UI/utilities), `src/features/{auth,landing,admin,worker,employer}` (domain-owned shells, components, sections, pages, hooks, store, portal/i18n). API split into `src/api/{core,auth,system,worker,employer,jobs,admin}`. All worker pages live under `pages/<kebab>/<Name>Page.tsx` and all employer pages under `pages/<kebab>/<Name>Page.tsx` (postings + create-posting share `pages/postings/`). Behaviour unchanged; only file locations + import paths.
